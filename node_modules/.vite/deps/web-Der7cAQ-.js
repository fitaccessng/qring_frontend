...
//#region node_modules/@codetrix-studio/capacitor-google-auth/dist/esm/web.js
var GoogleAuthWeb = class extends WebPlugin {
	...
	loadScript() {
		...
		const scriptId = "gapi";
		if (document === null || document === void 0 ? void 0 : document.getElementById(scriptId)) return;
	...
	initialize(_options = {
		...
		if (!clientId) console.warn("GoogleAuthPlugin - clientId is empty");
		this.options = {
			clientId,
	...
	platformJsLoaded() {
		gapi.load("auth2", () => {
			...
			if (this.options.scopes.length) clientConfig.scope = this.options.scopes.join(" ");
	...
	async signIn() {
		return new Promise(async (resolve, reject) => {
			...
			try {
				...
				if (needsOfflineAccess) serverAuthCode = (await gapi.auth2.getAuthInstance().grantOfflineAccess()).code;
				else await gapi.auth2.getAuthInstance().signIn();
				const googleUser = gapi.auth2.getAuthInstance().currentUser.get();
				if (needsOfflineAccess) await googleUser.reloadAuthResponse();
...
//# sourceMappingURL=web-Der7cAQ-.js.map