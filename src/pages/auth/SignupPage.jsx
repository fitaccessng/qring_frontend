...
const IS_MOBILE_APP_BUILD = !shouldUseGoogleAuth();
...
export default function SignupPage() {
  ...
  const termsHref = IS_MOBILE_APP_BUILD ? TERMS_URL : "/terms";
  const formAction = useAsyncAction({
    debounceMs: 500,
    timeoutMs: 15000,
    timeoutMessage: "Signup is taking too long. Please try again.",
  });
  const googleAction = useAsyncAction({
    debounceMs: 800,
    timeoutMs: 20000,
    timeoutMessage: "Google signup is taking too long. Please try again.",
  });
  ...
  useEffect(() => {
    ...
    return () => { active = false; };
  }, [googleAuthEnabled, navigate, resumeGoogleRedirect]);

  const onSubmit = async (event) => {
    event.preventDefault();
    try {
      return await formAction.run(async () => {
        if (!agreedToTerms) {
          ...
          <form onSubmit={onSubmit} className="flex flex-col gap-4 w-full">
            ...
            <div className="flex items-center gap-3 px-1 pt-2">
              ...
              <label htmlFor="terms" className="text-xs font-bold text-slate-500 uppercase tracking-tight cursor-pointer">
                Agree to{" "}
                {IS_MOBILE_APP_BUILD ? (
                  <a href={termsHref} target="_blank" rel="noreferrer" className="text-brand-500 underline underline-offset-2">
                    Terms & Conditions
                  </a>
                ) : (
                  <Link to={termsHref} className="text-brand-500 underline underline-offset-2">
                    Terms & Conditions
                  </Link>
                )}
              </label>
            ...
            <button
              disabled={submitting || formAction.pending}
          ...
          {googleAuthEnabled ? (
            <>
              ...
              <button
                ...
                disabled={submitting || googleAction.pending}
              ...
              >
                ...
                <span className="text-slate-700 font-bold text-sm">
                  {submitting && googleAction.pending ? "Opening Google..." : "Sign up with Google"}
                </span>
              </button>
            </>
          ) : null}
