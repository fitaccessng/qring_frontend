...
export default function NotificationBell({ unreadCount, onClick, isOpen = false }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group relative grid h-11 w-11 place-items-center overflow-hidden rounded-xl border transition-all ${
        isOpen
    ...
    >
      ...
      {unreadCount > 0 ? (
        <span className="absolute -right-1 -top-1 inline-flex min-w-[1.35rem] items-center justify-center rounded-full bg-rose-600 px-1 py-0.5 text-[10px] font-black leading-none text-white shadow-lg ring-2 ring-white dark:bg-rose-500 dark:text-white dark:ring-slate-900">
